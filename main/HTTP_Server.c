...
char complete_command[Max_Command_Length]; //1 KB

//* HTTP Post Handler for our commands
static esp_err_t command_post_handler(httpd_req_t *req)
{
    char buf[255];
    memset(complete_command, 0, Max_Command_Length);
    uint16_t data_idx = 0;
    int ret, remaining = req->content_len;
    ...
    while (remaining > 0) {
        ...
        ESP_LOGI(TAG, "=========== RECEIVED DATA ==========");
        ESP_LOGI(TAG, "%.*s", ret, buf);
        //save the data to the complete command variable
        for(uint16_t i=0; i< ret; i++){
			complete_command[data_idx] = buf[i];
			data_idx++;
		}
        ESP_LOGI(TAG, "====================================");
	...
	printf("Received total %d Bytes of Command\n", data_idx);
	printf("Complete Command is: %s\n", complete_command);
	parse_commands(complete_command);
	
	vTaskDelay(1000/portTICK_PERIOD_MS);
	//hopefully the command has been executed and the response is available in the global variable
	
	httpd_resp_send_chunk(req, global_http_response, strlen(global_http_response));
...
static esp_err_t ctrl_put_handler(httpd_req_t *req)
{
    ...
    if (buf == '0') {
        ...
        httpd_unregister_uri(req->handle, "/command");
        ...
        httpd_register_uri_handler(req->handle, &command);
